...
#: Phase 11 team/admin/enterprise tables (migration 0010) — additive only.
ENTERPRISE_TABLES = {
...
def test_0010_backfill_is_safe_and_preserves_ownership(migration_db: Path):
    ...
    command.upgrade(_alembic_config(migration_db), "0009_analytics_reporting")
    ...
    try:
        ...
        # downgrade removes ONLY what 0010 created (probe data still intact)
    finally:
        con.close()

    command.downgrade(_alembic_config(migration_db), "0009_analytics_reporting")
    con = sqlite3.connect(migration_db)
    try:
        assert _user_exists(migration_db, probe_id)
        assert con.execute("SELECT COUNT(*) FROM leads").fetchone()[0] == leads_before
